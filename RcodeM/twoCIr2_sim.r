twoCIr2_sim <-
function(ro2=0.2,n=15,p=2,lambda=0.95,nSim=50000,see=0,ss=3)
{
#Two-sided CI for multiple square correlation coefficient ro^2
dump(c("twoCIr2_sim","twoCIr2"),"c:\\Projects\\Mode\\twoCIr2_sim.r")
set.seed(ss)
t0=Sys.time()
A=matrix(runif(p^2,min=-2,max=8),ncol=p)
OM=t(A)%*%A
chOM=chol(OM)
beta=runif(n=p)
s2y=t(beta)%*%OM%*%beta/ro2
s2eps=(1-ro2)*s2y
matCI=matrix(ncol=10,nrow=nSim)
for(isim in 1:nSim)
{
	X=matrix(rnorm(n*p),ncol=p,nrow=n)%*%chOM-4
	y=X%*%beta+rnorm(n,sd=sqrt(s2eps))+1
	o=lm(y~X)
	r2=1-sum(o$residuals^2)/var(y)/(n-1)		
	v=as.vector(t(as.matrix(twoCIr2(r2=r2,n=n,p=p,see=see))))
	matCI[isim,]=v
}	
#print(matCI)	
covpr=width=rep(NA,4)
for(i in 1:5) 
{
	covpr[i]=mean(matCI[,2*i-1]<ro2 & ro2<matCI[,2*i])
	width[i]=mean(matCI[,2*i]-matCI[,2*i-1])
}
out.sim=as.data.frame(cbind(covpr,width))
row.names(out.sim)=c("Lower-sided","Upper-sided","Equal-tail","Unbiased","MBESS")
names(out.sim)=c("CovProb","Width")
print(out.sim)
Sys.time()-t0
}
twoCIr2 <-
